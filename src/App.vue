...
async function getAppid() {
  ...
  hasNoPersonalInfoRisk.value = false;
  try {
    const response = await xhsApi(link, turnstileToken.value);
    if (!response.success) {
    ...
    <section class="flex min-h-screen min-w-0 items-center justify-center">
      <div class="flex w-full max-w-md flex-col">
        <div class="flex items-center justify-center gap-2">
          ...
          <a href="https://github.com/llds66/unlink" target="_blank" rel="noopener noreferrer"
        ...
        <form class="mt-6 flex gap-2" @submit.prevent="getAppid">
          <div class="relative min-w-0 flex-1">
            <input v-model="url" type="text" autocomplete="off" placeholder="输入小红书APP分享链接或者内容" aria-label="小红书链接或分享文案"
            ...
            <button v-if="url" type="button" aria-label="清空链接"
          ...
          <button type="submit" :disabled="isLoading || !turnstileToken"
            ...
            <span v-if="isLoading" class="i-svg-spinners-3-dots-bounce text-xl" aria-label="加载中" />
            <span v-else>检测链接</span>
          </button>
        </form>

        <div class="mt-5 flex h-11 items-center justify-center px-4">
          <a v-if="appid && url" :href="`https://www.xiaohongshu.com/user/profile/${appid}`" target="_blank"
